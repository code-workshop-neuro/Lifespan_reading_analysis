...
coefficients_list$corrected_area <- ifelse(coefficients_list$adjusted_p_value_area < 0.05, coefficients_list$t_value_area, NA)

coefficients_list$adjusted_p_value_thickness <- p.adjust(coefficients_list$p_value_thickness, method = "fdr")
coefficients_list$corrected_thickness <- ifelse(coefficients_list$adjusted_p_value_thickness < 0.05, coefficients_list$t_value_thickness, NA)

coefficients_list$adjusted_p_value_volume <- p.adjust(coefficients_list$p_value_volume, method = "fdr")
coefficients_list$corrected_volume <- ifelse(coefficients_list$adjusted_p_value_volume < 0.05, coefficients_list$t_value_volume, NA)

coefficients_list$adjusted_p_value_lgi <- p.adjust(coefficients_list$p_value_lgi, method = "fdr")
coefficients_list$corrected_lgi <- ifelse(coefficients_list$adjusted_p_value_lgi < 0.05, coefficients_list$t_value_lgi, NA)
...
coefficients_thickness$corrected_t_list <- ifelse(coefficients_thickness$adjust_p_value_list < 0.05, coefficients_thickness$t_thickness_list, NA)
coefficients_thickness$adjust_p_value_sentence <- p.adjust(coefficients_thickness$p_thickness_sentence,method = "fdr")
coefficients_thickness$corrected_t_sentence <- ifelse(coefficients_thickness$adjust_p_value_sentence < 0.05, coefficients_thickness$t_thickness_sentence, NA)

coefficients_area$adjust_p_value_list <- p.adjust(coefficients_area$p_area_list,method = "fdr")
coefficients_area$corrected_t_list <- ifelse(coefficients_area$adjust_p_value_list < 0.05, coefficients_area$t_area_list, NA)
coefficients_area$adjust_p_value_sentence <- p.adjust(coefficients_area$p_area_sentence,method = "fdr")
coefficients_area$corrected_t_sentence <- ifelse(coefficients_area$adjust_p_value_sentence < 0.05, coefficients_area$t_area_sentence, NA)

coefficients_volume$adjust_p_value_list <- p.adjust(coefficients_volume$p_volume_list,method = "fdr")
coefficients_volume$corrected_t_list <- ifelse(coefficients_volume$adjust_p_value_list < 0.05, coefficients_volume$t_volume_list, NA)
coefficients_volume$adjust_p_value_sentence <- p.adjust(coefficients_volume$p_volume_sentence,method = "fdr")
coefficients_volume$corrected_t_sentence <- ifelse(coefficients_volume$adjust_p_value_sentence < 0.05, coefficients_volume$t_volume_sentence, NA)

coefficients_lgi$adjust_p_value_list <- p.adjust(coefficients_lgi$p_lgi_list,method = "fdr")
coefficients_lgi$corrected_t_list <- ifelse(coefficients_lgi$adjust_p_value_list < 0.05, coefficients_lgi$t_lgi_list, NA)
coefficients_lgi$adjust_p_value_sentence <- p.adjust(coefficients_lgi$p_lgi_sentence,method = "fdr")
coefficients_lgi$corrected_t_sentence <- ifelse(coefficients_lgi$adjust_p_value_sentence < 0.05, coefficients_lgi$t_lgi_sentence, NA)
...
for (i in indices) {
  ...
  for(group in names(grouped_data)) {
    ...
    pcor_test <- pcor.test(group_data[[col_name]], group_data$list,group_data[,c("age_at_test","sex","nonverbal")])
...
for (i in 2:69) {
  ...
  model <- lm(msn_data_grouped[[brain_region]] ~ sex, data = msn_data_grouped)
...
for (i in 1:ncol(msn_degree)) {
  model <- lm(msn_degree[, i] ~ cleaned_df$age_at_test+cleaned_df$sex)
  model_list <- lm(msn_degree[, i] ~ cleaned_df$list+cleaned_df$age_at_test+cleaned_df$sex+cleaned_df$nonverbal)
  model_sentence <- lm(msn_degree[, i] ~ cleaned_df$sentence+cleaned_df$age_at_test+cleaned_df$sex+cleaned_df$nonverbal)
...
coefficients_list$age_t_value_corrected <- ifelse(coefficients_list$age_p_value_corrrected < 0.05, coefficients_list$msndegree_age_t_values, NA)
coefficients_list$list_p_value_corrected <- p.adjust(coefficients_list$msndegree_list_p_values,method = "fdr")
coefficients_list$list_t_value_sig <- ifelse(coefficients_list$msndegree_list_p_values < 0.06, coefficients_list$msndegree_list_t_values, NA)
coefficients_list$sentence_p_value_corrected <- p.adjust(coefficients_list$msndegree_sentence_p_values,method = "fdr")
coefficients_list$sentence_t_value_sig <- ifelse(coefficients_list$msndegree_sentence_p_values < 0.06, coefficients_list$msndegree_sentence_t_values, NA)
